...
from app.services.temporal_domain_service import canonical_event_key, reconcile_calendar_events, temporal_event_state
...
def event(*, release: datetime, name: str = "Consumer Price Index", actual=None) -> EconomicEvent:
    return EconomicEvent(
        event_id="cpi-1", name=name, country="US", category="CPI", date=release.date().isoformat(),
        metric_id="headline_cpi_mom", normalized_event_family="CPI",
        reference_period=release.strftime("%Y-%m"), frequency="monthly",
        time_utc=release, impact=Impact.HIGH, actual=actual, source="BLS", source_url="https://bls.gov/cpi",
        reliability=0.99, event_risk_level=Impact.HIGH,
        enrichment=EventEnrichment(
            forecast="0.3", consensus="0.3", previous="0.2", actual=actual,
            metrics=[{
                "metric_id": "headline_cpi_mom", "period": release.strftime("%Y-%m"),
...
def test_job_queue_is_persistent_idempotent_and_recovers_expired_lease(tmp_path: Path) -> None:
    ...
    first, created = service.enqueue_explicit(
        job_type="MISSING_EVENT_RESEARCH", symbol="MNQ", correlation_id="corr-1",
        request_payload={"pending_fields": ["forecast"]}, pending_fields=["forecast"],
    )
    duplicate, created_again = service.enqueue_explicit(
        job_type="MISSING_EVENT_RESEARCH", symbol="MNQ", correlation_id="corr-2",
        request_payload={"pending_fields": ["forecast"]}, pending_fields=["forecast"],
    )
...
def test_release_retry_backoff_is_exact_and_survives_repository_reopen(tmp_path: Path) -> None:
    ...
    repo = AIResearchJobRepository(cfg, clock=lambda: now[0])
    job, _ = repo.enqueue(
        idempotency_key="release", job_type="RELEASE_ACTUAL_REFRESH", symbol="MNQ",
        correlation_id="release-corr", event_key="event:1", request_payload={},
        policy_version="source-policy-v1", prompt_version="v1", max_attempts=7,
    )
    observed = []
    for delay in (30, 120, 300, 900, 1800, 3600):
        running = repo.acquire_next("worker")
        assert running and running["job_id"] == job["job_id"]
        retried = repo.retry_or_fail(job["job_id"], "worker", error="not yet", delays=[30, 120, 300, 900, 1800, 3600])
...
def test_temporal_event_states_reject_future_actual_and_handle_speech(tmp_path: Path) -> None:
    ...
    with connect_market_db(facts.settings) as conn:
        assert conn.execute("SELECT actual FROM economic_events_history").fetchone()["actual"] is None
...
def test_xtb_event_becomes_canonical_and_discordant_values_are_preserved() -> None:
    ...
    payload = {
        "source": "XTB Economic Calendar", "source_url": "https://xtb.com/calendar",
        "items": [{
            "country": "US", "event_name": base.name, "category": "CPI", "release_at": release.isoformat(),
            "date": release.date().isoformat(), "forecast": "0.4", "consensus": "0.4", "previous": "0.2",
        }],
    }
...
def test_worker_uses_unique_workspaces_and_never_waits_in_http_path(tmp_path: Path) -> None:
    ...
    for index in range(2):
        service.enqueue_explicit(
            job_type="MISSING_EVENT_RESEARCH", symbol="MNQ", correlation_id=f"corr-{index}",
            request_payload={"pending_fields": [f"field-{index}"]}, pending_fields=[f"field-{index}"],
        )
    seen: list[Path] = []
...
def test_official_actual_worker_updates_history_and_fact_with_surprise(tmp_path: Path, monkeypatch) -> None:
    cfg = settings(tmp_path)
    ...
    debug = {
        ...
        "market_schedule": {"status": "AVAILABLE", "context_date": released.date, "market_session_status": "open"},
        "event_calendar": {"critical_macro_events": [released.model_dump(mode="json")], "fed_communications": [], "other_economic_events": []},
        "events_today": [], "macro_snapshot": {}, "risk_context": {}, "nasdaq_context": {}, "news_context": {},
        "rates_expectations": {}, "positioning": {}, "sentiment_context": {}, "data_quality": {},
    }
    snapshots.save_next(
        symbol="MNQ", refresh_mode="auto", debug_payload=debug,
        ai_enrichment={"status": "PENDING", "job_ids": [jobs[0]["job_id"]]},
        source_job_id=jobs[0]["job_id"], job_ids=[jobs[0]["job_id"]],
    )

    async def official_bls_fetch(_self):
        return ProviderResult(
            metadata=ProviderMetadata(
                source="BLS", provider_type=ProviderType.API,
                retrieved_at=datetime.now(UTC), data_as_of=released.time_utc.replace(day=1),
                freshness=Freshness.RECENT, reliability=0.99,
            ),
            data={"CUSR0000SA0": {
                "value": 101.5, "data_as_of": released.time_utc.strftime("%Y-%m-01"),
                ...
                "source_domain": "bls.gov", "provider_adapter": "BLS_OFFICIAL_API",
                "official_adapter": True,
                "observations": [
                    {"period": (released.time_utc.replace(day=1) - timedelta(days=1)).strftime("%Y-%m"), "value": 101.0},
                    {"period": released.time_utc.strftime("%Y-%m"), "value": 101.5},
                ],
            }},
        )

    monkeypatch.setattr("app.services.deterministic_actual_resolver.BlsProvider.fetch", official_bls_fetch)
    ...
    with connect_market_db(cfg) as conn:
        ...
        fact_row = conn.execute("SELECT * FROM market_facts WHERE canonical_event_key=?", (key,)).fetchone()
    assert row["actual"] == "0.5" and row["forecast"] == "0.3" and row["consensus"] == "0.3" and row["previous"] == "0.2"
...
def test_expired_news_is_historical_not_current(tmp_path: Path) -> None:
    ...
    repository.upsert_news({
        "title": "CPI release recap", "summary": "Published result", "source": "Reuters",
        "source_url": "https://reuters.com/example", "published_at": "2026-01-01T12:00:00Z",
        "retrieved_at": "2026-01-01T12:01:00Z", "valid_until": "2026-01-02T00:00:00Z",
        "topics": ["inflation"], "symbols": ["MNQ"],
    })
    assert repository.current(days=365) == []
    historical = repository.stored(days=365)
    assert historical and historical[0]["lifecycle_status"] == "EXPIRED" and historical[0]["historical"] is True
...
def test_consumer_exposes_structured_ai_and_blocks_full_readiness_when_pending() -> None:
    full = {
        ...
        "snapshot_id": "mcs-1", "snapshot_revision": 3,
        "market_schedule": {"status": "AVAILABLE", "context_date": "2026-07-22", "market_session_status": "open"},
        ...
        "events_today": [], "event_windows": {}, "risk_context": {"status": "AVAILABLE", "VIX": {"value": 16}},
        "nasdaq_context": {"qqq_holdings": {"holdings_count": 100}, "earnings": {}},
        "news_context": {"status": "NO_RELEVANT_NEWS", "latest": []}, "rates_expectations": {},
        "positioning": {}, "sentiment_context": {}, "data_quality": {}, "quality": {},
        "ai_enrichment": {"status": "PENDING", "job_ids": ["airj-1"], "pending_fields": ["consensus"]},
    }
...
def test_additive_migration_from_v6_preserves_existing_rows(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db) as conn:
        ...
        conn.commit()
    assert migrate_database(db)["schema_version"] == 10
    with sqlite3.connect(db) as conn:
