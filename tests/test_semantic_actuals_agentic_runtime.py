...
def settings(tmp_path: Path, **overrides) -> Settings:
    values = {
        "database_path": tmp_path / "market.sqlite", "source_policy_path": POLICY,
        "ai_job_workspace_root": tmp_path / "jobs", "enable_ai_researcher": True,
...
def observations(values: list[str], *, year: int = 2025, start_month: int = 7) -> list[dict]:
    ...
    for index, value in enumerate(values):
        ...
        output.append({
            ...
            "value": value, "release_vintage": f"v{index + 1}",
        })
    return output
...
def derive(metric_id: str, values: list[str], period: str, **series) -> dict:
    ...
    return derive_official_actual(
        spec, {"observations": observations(values), **series}, expected_period=period,
        retrieved_at="2026-07-22T10:00:00+00:00", release_timestamp="2026-07-22T08:30:00+00:00",
    )


def observe_sources(repository: ResearchRuntimeRepository, run: dict, urls: list[str]) -> None:
    step, _ = repository.begin_step(
        run["run_id"], "OPEN_SOURCE", 3, {}, backend="fake", tool="fake_open",
    )
    repository.record_tool_events(
        run["run_id"], step["step_id"],
        [
            {
                "event_type": "open_source", "source_url": url, "canonical_url": url,
                "observed_at": "2026-07-22T10:00:00Z", "content_hash": f"hash-{index}",
                "http_status": 200,
            }
...
def test_gdp_uses_official_annualized_rate_not_chained_dollar_level() -> None:
    ...
    actual = derive_official_actual(
        spec, {"observations": [{"period": "2026Q1", "value": "3.1", "release_vintage": "third-estimate"}]},
        expected_period="2026-Q1", retrieved_at="2026-06-25T12:30:00Z",
...
def test_latest_release_vintage_is_used_and_revision_is_preserved() -> None:
    ...
    actual = derive_official_actual(
        ...
        expected_period="2026-06", retrieved_at="2026-07-22T10:00:00Z",
...
def event(release: datetime, *, baseline_unit: str = "percent", baseline_period: str = "2026-06") -> EconomicEvent:
    return EconomicEvent(
        event_id="cpi-release", name="CPI (MoM)", country="US", category="CPI",
        metric_id="headline_cpi_mom", normalized_event_family="CPI",
        reference_period="2026-06", frequency="monthly", date=release.date().isoformat(),
        time_utc=release, impact=Impact.HIGH, source="BLS", source_url="https://www.bls.gov/cpi/",
        reliability=0.99, event_risk_level=Impact.HIGH,
        enrichment=EventEnrichment(
            forecast="0.4", consensus="0.4", previous="0.2",
            metrics=[{
                "metric_id": "headline_cpi_mom", "period": baseline_period,
                "frequency": "MoM", "unit": baseline_unit, "consensus": "0.4",
            }],
        ),
    )
...
def bls_result(retrieved_at: datetime) -> ProviderResult:
    return ProviderResult(
        metadata=ProviderMetadata(
            source="BLS", provider_type=ProviderType.API, retrieved_at=retrieved_at,
            data_as_of=datetime(2026, 6, 1, tzinfo=UTC), freshness=Freshness.RECENT, reliability=0.99,
        ),
        data={"CUSR0000SA0": {
            "units": "index", "frequency": "monthly", "seasonal_adjustment": "SA",
            "source": "BLS", "source_url": "https://api.bls.gov/publicAPI/v2/timeseries/data/",
            "canonical_url": "https://www.bls.gov/developers/api_signature_v2.htm",
            "source_domain": "bls.gov", "provider_adapter": "BLS_OFFICIAL_API", "official_adapter": True,
            "observations": [
...
def test_worker_persists_semantic_actual_and_only_computes_compatible_surprise(
    tmp_path: Path, monkeypatch, baseline_unit: str, baseline_period: str,
    compatible: bool, warning: str | None,
) -> None:
    ...
    with connect_market_db(cfg) as conn:
        row = conn.execute("SELECT * FROM economic_events_history WHERE canonical_event_key=?", (key,)).fetchone()
        candidate = conn.execute("SELECT * FROM event_value_candidates WHERE canonical_event_key=?", (key,)).fetchone()
    assert row["actual"] == "0.5" and bool(row["actual_semantic_compatible"]) is compatible
    assert row["actual_metric_id"] == "headline_cpi_mom" and candidate["source_series_id"] == "CUSR0000SA0"
    if compatible:
        assert row["surprise_value"] == "0.1" and row["surprise_direction"] == "above_consensus"
    else:
        assert row["surprise_value"] is None and warning in json.loads(row["semantic_warnings_json"])
...
def test_capability_ready_to_smoke_and_degraded_with_fake_executable(tmp_path: Path) -> None:
    ...
    cfg = settings(
        tmp_path / "ready-db", codex_cli_command=str(ready_command), ai_worker_enabled=True,
        ai_research_web_access_enabled=True,
    )
    report = AIResearchCapabilityService(cfg).probe()
    assert report["status"] == "READY_TO_SMOKE" and report["executable_version"] == "codex-cli 9.9.9-fake"
    ...
    AIResearchCapabilityService(cfg).record_live_verification({
        "observed_search_count": 1, "opened_source_count": 1,
        "source_domains": ["example.gov"], "run_id": "offline-fixture-proof",
    })
...
def test_fake_agent_runs_all_persistent_steps_and_persists_claim_evidence(tmp_path: Path) -> None:
    ...
    cfg = settings(
        tmp_path, codex_cli_command=str(command), ai_worker_enabled=True,
        ai_research_web_access_enabled=True,
    )
    AIResearchCapabilityService(cfg).record_live_verification({
        "observed_search_count": 1, "opened_source_count": 2,
        "source_domains": ["bloomberg.com", "ft.com"], "run_id": "fixture-live-run",
    })
    ...
    assert steps == [(name, "COMPLETED") for name in (
...
def test_agentic_runtime_resumes_after_mid_research_crash(tmp_path: Path) -> None:
    ...
    job, _ = service.enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="resume",
...
def test_job_storm_is_prevented_after_terminal_in_same_run_window(tmp_path: Path, terminal: str) -> None:
...
def test_scheduler_fingerprint_returns_not_required_and_supports_event_triggers(tmp_path: Path) -> None:
    cfg = settings(tmp_path, research_daily_budget_runs=10)
    MarketContextSnapshotRepository(cfg).save_next(
        symbol="MNQ", refresh_mode="test",
        debug_payload={
            "symbol": "MNQ", "generated_at_utc": "2026-07-22T10:00:00Z",
            "market_schedule": {"context_date": "2026-07-22", "market_session_status": "open"},
            "event_calendar": {}, "news_context": {}, "quality": {}, "data_quality": {},
        },
    ...
    assert second["decision"] == "NOT_REQUIRED" and second["reason"] == "input_fingerprint_unchanged"
...
def test_scheduler_pre_event_and_post_release_enqueue_exact_event_work(tmp_path: Path) -> None:
    ...
    MarketContextSnapshotRepository(cfg).save_next(
        symbol="MNQ", refresh_mode="test",
        debug_payload={
            "symbol": "MNQ", "generated_at_utc": datetime.now(UTC).isoformat(),
            "market_schedule": {"context_date": "2026-07-22", "market_session_status": "open"},
            "event_calendar": {
                "critical_macro_events": [future.model_dump(mode="json"), released.model_dump(mode="json")],
                "fed_communications": [], "other_economic_events": [],
            },
            "news_context": {}, "quality": {}, "data_quality": {},
        },
        ai_enrichment={"status": "NOT_REQUIRED", "job_ids": []},
    )
    scheduler = ResearchSchedulerService(cfg)
...
def test_scheduler_ignores_new_snapshot_revision_with_same_substantive_content(tmp_path: Path) -> None:
    cfg = settings(tmp_path, research_daily_budget_runs=10)
    ...
    snapshots.save_next(
        symbol="MNQ", refresh_mode="test", debug_payload=debug,
        ai_enrichment={"status": "NOT_REQUIRED", "job_ids": []},
    )
    scheduler = ResearchSchedulerService(cfg)
    first = scheduler.evaluate("premarket")
    snapshots.save_next(
        symbol="MNQ", refresh_mode="test", debug_payload={**debug, "generated_at_utc": "2026-07-22T10:01:00Z"},
        ai_enrichment={"status": "NOT_REQUIRED", "job_ids": []},
    )
    second = scheduler.evaluate("premarket")
    assert first["decision"] == "QUEUED"
...
def test_event_outside_pre_event_window_does_not_enqueue(tmp_path: Path) -> None:
    cfg = settings(
        tmp_path, research_daily_budget_runs=10, research_pre_event_window_minutes=60,
    )
...
def test_additive_migration_from_v8_preserves_rows_and_adds_runtime(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(database) as conn:
        ...
        for version, (name, sql) in enumerate(MIGRATIONS[:8], start=1):
            ...
            conn.execute("INSERT INTO schema_migrations VALUES (?,?,?)", (version, name, "2026-07-22"))
...
def test_additive_migration_from_v9_preserves_rows_and_adds_verified_runtime(tmp_path: Path) -> None:
...
def test_tool_usage_and_cost_are_persisted_only_when_cli_provides_them(tmp_path: Path) -> None:
    ...
    job, _ = service.enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="usage-provided",
        request_payload={"database_context": {}},
    )
    run = repository.ensure_run(job, "MNQ_MARKET_RESEARCH", "mnq_market_research_v1")
    step, _ = repository.begin_step(
        run["run_id"], "SEARCH", 2, {}, backend="fixture", tool="fixture",
    )
    repository.record_tool_events(
        run["run_id"], step["step_id"],
    ...
    no_usage_job, _ = service.enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="usage-unavailable",
        request_payload={"database_context": {"revision": 2}}, force=True,
    )
    no_usage_run = repository.ensure_run(
        no_usage_job, "MNQ_MARKET_RESEARCH", "mnq_market_research_v1",
    )
    no_usage_step, _ = repository.begin_step(
        no_usage_run["run_id"], "SEARCH", 2, {}, backend="fixture", tool="fixture",
    )
    repository.record_tool_events(
        no_usage_run["run_id"], no_usage_step["step_id"],
        [{"event_type": "search", "query": "another bounded query"}],
    )
    with sqlite3.connect(cfg.database_path) as conn:
        row = conn.execute(
            "SELECT usage_json,cost_json FROM research_runs WHERE run_id=?", (no_usage_run["run_id"],)
        ).fetchone()
...
def test_evidence_deduplicates_exact_rows_and_requires_distinct_content(tmp_path: Path) -> None:
    ...
    repository = ResearchRuntimeRepository(cfg)
    job, _ = service.enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="dedup",
        request_payload={"database_context": {"revision": 1}},
    )
    run = repository.ensure_run(job, "MNQ_MARKET_RESEARCH", "mnq_market_research_v1")
    base = {
        "topic": "macro", "field_semantics": "consensus", "value": "0.3",
        "metric_id": "headline_cpi_mom", "period": "2026-06", "frequency": "monthly", "unit": "percent",
    }
    bloomberg = {
        "source_url": "https://www.bloomberg.com/cpi", "publisher": "Bloomberg",
        "evidence_text": "Bloomberg survey consensus is 0.3 percent.", "retrieved_at": "2026-07-22T10:00:00Z",
    }
    ft = {
        "source_url": "https://www.ft.com/cpi", "publisher": "Financial Times",
        "evidence_text": "FT survey independently reports 0.3 percent.", "retrieved_at": "2026-07-22T10:00:00Z",
    }
    ...
    second_job, _ = service.enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="syndicated",
        request_payload={"database_context": {"revision": 2}}, force=True,
    )
    ...
    rejected = repository.persist_claims(second_run, [{
        **base, "evidence": [
...
def test_claim_evidence_recalculates_tier_and_rejects_invented_confirmation(tmp_path: Path) -> None:
    ...
    job, _ = AIResearchJobService(cfg).enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="evidence",
        request_payload={"database_context": {}},
    )
    repository = ResearchRuntimeRepository(cfg)
    run = repository.ensure_run(job, "MNQ_MARKET_RESEARCH", "mnq_market_research_v1")
    claim = {
        "topic": "macro", "field_semantics": "consensus", "value": "0.3",
        "metric_id": "headline_cpi_mom", "period": "2026-06", "frequency": "monthly",
        "unit": "percent", "confirmation_count": 99, "source_tier": 1,
        "evidence": [{
            "source_url": "https://www.investing.com/cpi", "publisher": "Investing",
            "evidence_text": "Consensus is 0.3 percent.", "retrieved_at": "2026-07-22T10:00:00Z",
        }],
    }
    ...
    with sqlite3.connect(cfg.database_path) as conn:
        evidence = conn.execute("SELECT source_tier,source_domain FROM research_evidence").fetchone()
...
def test_partial_when_only_some_claims_pass_evidence_policy(tmp_path: Path) -> None:
    cfg = settings(tmp_path)
    job, _ = AIResearchJobService(cfg).enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="partial",
        request_payload={"database_context": {}},
    )
    repository = ResearchRuntimeRepository(cfg)
    run = repository.ensure_run(job, "MNQ_MARKET_RESEARCH", "mnq_market_research_v1")
    valid = {
        "topic": "risk", "field_semantics": "exploratory_context", "value": "verified context",
        "evidence": [{
            "source_url": "https://www.reuters.com/markets/a", "publisher": "Reuters",
            "evidence_text": "Verified market context.", "retrieved_at": "2026-07-22T10:00:00Z",
        }],
    }
    invalid = {
        "topic": "risk", "field_semantics": "news", "value": "unsupported",
        "evidence": [{
            "source_url": "https://unknown.example/a", "publisher": "Unknown",
            "evidence_text": "Unsupported claim.", "retrieved_at": "2026-07-22T10:00:00Z",
        }],
    }
...
def test_fred_fallback_is_never_promoted_to_official_bls_actual(tmp_path: Path, monkeypatch) -> None:
    cfg = settings(tmp_path)
    ...
    async def fred_fallback(_self):
        return ProviderResult(
            metadata=ProviderMetadata(
                source="FRED fallback for unavailable BLS transport", provider_type=ProviderType.API,
                retrieved_at=datetime.now(UTC), data_as_of=datetime(2026, 6, 1, tzinfo=UTC),
                freshness=Freshness.RECENT, reliability=0.9, is_fallback=True,
            ),
            data={"CUSR0000SA0": {
                "observations": observations(["300", "301.5"], year=2026, start_month=5),
                "frequency": "monthly", "seasonal_adjustment": "SA", "units": "index",
                "source": "FRED", "source_url": "https://api.stlouisfed.org/fred/series/observations",
                "canonical_url": "https://fred.stlouisfed.org/series/CPIAUCSL",
                "source_domain": "fred.stlouisfed.org", "provider_adapter": "FRED_FALLBACK_API",
                "official_adapter": False,
            }},
        )

    monkeypatch.setattr("app.services.deterministic_actual_resolver.BlsProvider.fetch", fred_fallback)
...
def test_official_feed_delay_remains_retryable_beyond_old_two_hour_window(tmp_path: Path) -> None:
    ...
    worker = AIResearchWorker(cfg, facts=facts, actual_resolver=delayed, worker_id="delayed-official")
    assert worker.process_once()
    restored = AIResearchJobRepository(cfg).get(job["job_id"])
    assert restored["status"] == "RETRY_SCHEDULED"
    assert restored["retry_class"] == "OFFICIAL_ACTUAL"
    assert datetime.fromisoformat(restored["retry_deadline_at"]) > datetime.now(UTC) + timedelta(hours=20)
...
class ClaimExecutor:
    ...
    def execute_step(self, **kwargs):
        ...
        if step == "OPEN_SOURCE" and self.observed:
            return {
                ...
                "_tool_events": [
                    {
                        "event_type": "open_source", "source_url": url, "canonical_url": url,
                        "content_hash": f"hash-{index}", "http_status": 200,
        ...
        if step == "VALIDATE":
            return {"claims": [{
                "topic": "macro", "field_semantics": "consensus", "value": "0.3",
                "metric_id": "headline_cpi_mom", "period": "2026-06",
                "frequency": "monthly", "unit": "percent",
                "evidence": [
                    {
                        "source_url": "https://www.bloomberg.com/verified", "publisher": "Bloomberg",
                        "evidence_text": "Verified consensus is 0.3 percent.",
                    },
                    {
                        "source_url": "https://www.ft.com/verified", "publisher": "Financial Times",
...
def test_ai_evidence_requires_actual_open_source_telemetry(
    tmp_path: Path, observed: bool, accepted_count: int,
) -> None:
    cfg = settings(tmp_path)
    job, _ = AIResearchJobService(cfg).enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id=f"observed-{observed}",
        request_payload={"database_context": {}},
    )
...
def _topic_claim(topic: str) -> dict:
    return {
        "topic": topic, "field_semantics": "exploratory_context", "value": f"verified {topic}",
        "evidence": [{
            "source_url": "https://www.reuters.com/markets/topic-coverage", "publisher": "Reuters",
...
def test_topic_completeness_is_server_calculated_for_partial_and_success(tmp_path: Path) -> None:
    ...
    repository = ResearchRuntimeRepository(cfg)
    job, _ = service.enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="one-topic",
    ...
    full_job, _ = service.enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="all-topics",
        request_payload={"database_context": {"version": 2}}, force=True,
    )
    ...
    assert complete["accepted_count"] == complete["persisted_count"] == complete["read_back_count"] == 10
...
def test_agentic_deadline_is_global_not_reapplied_per_phase(tmp_path: Path) -> None:
    cfg = settings(tmp_path)
    job, _ = AIResearchJobService(cfg).enqueue_explicit(
        job_type="MNQ_MARKET_RESEARCH", symbol="MNQ", correlation_id="deadline",
        request_payload={"database_context": {}},
    )
